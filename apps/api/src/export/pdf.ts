...
import { renderChartPng } from './png.js';

const MARGIN = 36;
const [PAGE_WIDTH, PAGE_HEIGHT] = PageSizes.Letter;

/**
 * Above this many stitches, the chart is drawn as an embedded raster (one bounded PNG per
 * page-tile) instead of one vector rectangle + glyph per cell. pdf-lib buffers every operator
 * in memory before serializing, so per-cell vectors are O(cells) memory and time — a full
 * 800x800 grid (640k cells) exhausts heap. The raster path keeps the page tiling, numbering,
 * and gridlines but replaces the interior fills with an image whose size the PNG renderer
 * already bounds. Chosen well below the measured danger zone (400x400 ≈ 530 MB).
 */
